...
class HtmlUnitLocatorLabelTest < HtmlUnitLocatorTest
  ...
  def test_locates_the_element_with_the_shortest_matching_for_attribute
    dom = dom('<label for="foo-bar"></label><label for="foo"></label>')
    element = Locators::Label.new(dom, 'foo').locate
    assert_equal 'foo', element.attribute('for')
  end
